...
// ---------------------------------------------------------------------------
  ...
  inline void operator delete  (void* p, const std::nothrow_t&) noexcept { mi_free(p); }
  inline void operator delete[](void* p, const std::nothrow_t&) noexcept { mi_free(p); }

  inline mi_decl_new(n) void* operator new(std::size_t n) noexcept(false) { return mi_new(n); }
  inline mi_decl_new(n) void* operator new[](std::size_t n) noexcept(false) { return mi_new(n); }

  inline mi_decl_new_nothrow(n) void* operator new  (std::size_t n, const std::nothrow_t& tag) noexcept { (void)(tag); return mi_new_nothrow(n); }
  inline mi_decl_new_nothrow(n) void* operator new[](std::size_t n, const std::nothrow_t& tag) noexcept { (void)(tag); return mi_new_nothrow(n); }
  ...
  inline void operator delete[](void* p, std::size_t n, std::align_val_t al) noexcept { mi_free_size_aligned(p, n, static_cast<size_t>(al)); };
  inline void operator delete  (void* p, std::align_val_t al, const std::nothrow_t&) noexcept { mi_free_aligned(p, static_cast<size_t>(al)); }
  inline void operator delete[](void* p, std::align_val_t al, const std::nothrow_t&) noexcept { mi_free_aligned(p, static_cast<size_t>(al)); }
